...
#define SYSCFG(type, name, val, ...) \
    CONFIG(type, name, val, .ini_change = zai_config_system_ini_change, ##__VA_ARGS__)
...
static bool _parse_uint(zai_string_view value, zval *nonnull decoded_value, long long max);

static bool _parse_uint32(zai_string_view value, zval *nonnull decoded_value, bool persistent) {
    UNUSED(persistent);
    return _parse_uint(value, decoded_value, UINT32_MAX);
}
static bool _parse_uint64(zai_string_view value, zval *nonnull decoded_value, bool persistent) {
    UNUSED(persistent);
...
#define CONFIG(type, name, ...) ZAI_CONFIG_ENTRY(DDAPPSEC_CONFIG_##name, name, type, __VA_ARGS__),
#define CALIAS(type, name, ...) ZAI_CONFIG_ALIASED_ENTRY(DDAPPSEC_CONFIG_##name, name, type, __VA_ARGS__),
...
static char _tolower_ascii(char c) { return c >= 'A' && c <= 'Z' ? c - ('A' - 'a') : c; }

static void _copy_tolower(char *restrict dst, const char *restrict src) {
    while (*src) {
        *(dst++) = _tolower_ascii(*(src++));
    }
}

static void dd_ini_env_to_ini_name(const zai_string_view env_name, zai_config_name *nonnull ini_name) {
