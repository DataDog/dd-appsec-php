...
{
    zend_string *client_ip_header = get_global_DD_TRACE_CLIENT_IP_HEADER();
    if (client_ip_header && ZSTR_LEN(client_ip_header) > 0) {
        zend_string *value = _fetch_arr_str(server, client_ip_header);
        if (!value) {
            return NULL;
        }
